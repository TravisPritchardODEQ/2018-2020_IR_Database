...
temp <- read.csv("//deqhq1/WQASSESSMENT/2018IRFiles/2018_WQAssessment/Final List/Data/Temperature_IR_data_ALLDATA.csv",
  ...
  rename(year_round_Excursion = year_round_Violation,
         Spawn_Excursion = Spawn_Violation )
...
biocriteria <- dbReadTable(con,"BioCriteria" ) %>%
  arrange(AU_ID, MLocID, SampleDate) %>%
  select(-TribalLand, -IndexID)
