...
server <- function(input, output, session) {
  ...
  #Get the ability to pass aparameter to the selectize inputs
  #modify url with "/?AU={AU_ID} and this will autopoulate the selectixe field with the selected AU
  observe({
    query <- parseQueryString(session$clientData$url_search)
    if (!is.null(query[['AU']])) {
      updateSelectizeInput(session, "AUs", selected = query[['AU']])
    }
  })
